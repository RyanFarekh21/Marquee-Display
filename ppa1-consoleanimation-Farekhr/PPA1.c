...
char cmd; // command charachter
char displayString[String_size] = {0};
int print_start; // index where rhe print should start
int input_pos; // the index where an input will be placed
int direction; //switching printing position. 0 - from right to left; 1 - from left to right
...
void Initialize(void)
{
    ...
    print_start = 0; // printing will start at the 3rd element
    direction = 0; // initialize direction to be right to left
    input_pos = 2; // inout will be placed as the 3rd element
...
void RunLogic(void)
{
    ...
    else if (((cmd >= 48 && cmd <= 57) || (cmd >= 65 && cmd <= 90) || (cmd >= 97 && cmd <= 122)) && (input_pos < String_size-2))
    {
        displayString[input_pos] = cmd;
        displayString[input_pos + 2] = '<'; 
        input_pos++;
    }
    else if (cmd == 32){
        if(direction == 0){
...
void DrawScreen(void)
{
    ...
    {
        int index = (print_start + i) % String_size;
        MacUILib_printf("%c", displayString[index]);
    } 
